...
export const addGliderportMarkers = (map) => {
  ...
  map.loadImage('./images/white-glider.png', (error, image) => {
    ...
    const isDefaultMap = !map.getStyle().name.toLowerCase().includes('satellite')
    ...
    map.addLayer({
      ...
      layout: {
        ...
        'text-size': 10, // set the size for the text
        'text-offset': [0, 2],
      ...
      paint: {
        'text-color': isDefaultMap ? '#000000' : '#ffffff', // set the color for the text
      },
    })
  })
}
